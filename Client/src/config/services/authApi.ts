...
export const login = async (role: string, values: object) => {
  const authApi = createAxiosInstance(role as AppEndpoint, role as AuthRole);
  try {
...
export const gLogin = async (response: object) => {
    const authApi = createAxiosInstance("user", "user");
    try {
        const res = await authApi.post('/google/login', response);
        return res.data;
    } catch (error) {
        throw error;
    }
...
export const getTypes = async () => {
  const authApi = createAxiosInstance("vendor", "vendor");
  try {
