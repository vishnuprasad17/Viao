  ...
  return (
     <div className="m-10">
       <div className="mb-5 flex justify-between md:flex-row flex-col gap-4">
         ...
         <Button variant="gradient" className="rounded-full md:mx-20 w-50" color="blue" placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
           <Link to={ADMIN.VENDOR_TYPES}>View Vendor Types</Link>
         </Button>
       </div>
       <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
         {vendors.map((vendor, index) => (
           <Link key={index} to={`${ADMIN.VENDOR}?Id=${vendor.id}`}>
             <VendorCard {...vendor} />
           </Link>
         ))}
       </div>
