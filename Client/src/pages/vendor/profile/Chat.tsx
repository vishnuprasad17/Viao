...
const Chat = () => {
  ...
  const receiverId = useMemo(() => {
    return currentChat?.members.find((m) => m !== vendor?.id) || null;
  }, [currentChat, vendor?.id]);
  ...
  // Load user data when chat changes
  useEffect(() => {
    if (!receiverId) {
      setUser(null);
      return;
    ...
    const loadUser = async () => {
      try {
        const res = await getUserForChat(receiverId);
        setUser(res.data);
      } catch (error) {
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const imageUrl = await uploadImageToS3(file, 'vendor');
  ...
  if (conversations.length === 0 && !vendor?.id) {
