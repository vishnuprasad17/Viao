...
import { deleteForEveryone, deleteForMe } from "../../../../config/services/venderApi";
...
const Message:React.FC<MessageProps>=({ message, own, setIsUpdated }) => {
  ...
  const handleDeleteForMe = async (side: string) => {
    deleteForMe(
        { msgId: messageIdToDelete, id: vendor?.id },
        { withCredentials: true }
      )
      .then((response) => {
        console.log(response);
          ...
          <div className="flex items-end justify-end">
            <div className="relative flex flex-col space-y-2 text-xs max-w-xs mx-2 order-2 items-end">
              <div>
                ...
                ) : message?.deletedIds?.includes(vendor?.id!) ? (
                  ""
                ) : message?.imageUrl ? (
                  <img
            ...
            <div className="relative flex flex-col space-y-2 text-xs max-w-xs mx-2 order-2 items-start">
              <div>
                {message?.isDeleted ? (
                  ...
                  <>
                    <span
                      className="px-4 py-2 rounded-lg inline-block rounded-bl-none bg-white text-gray-800"
