...
class VendorTypeService {
  async addType(type: string, status: string, image:string, imageUrl:string) {
    try {
      ...
      throw new BaseError("Failed to add new vendor type.", 500);
    }
  }
  ...
  async deleteType(vendorTypeId: string) {
    try {
      ...
      throw new BaseError("Failed to delete vendor type.", 500);
    }
  }
  ...
  async getSingleType(vendorTypeId: string) {
    try {
      ...
      throw new BaseError("Failed to retrieve vendor type.", 500); 
    }
  }
